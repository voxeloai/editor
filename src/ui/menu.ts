...
import logoSvg from "./svg/voxelo_logo_white.svg";
...
class Menu extends Container {
  constructor(events: Events, args = {}) {
    ...
    const icon = createSvg(logoSvg);
    icon.style.padding = "10px";
    icon.width = 30;
    icon.height = 30;
    icon.dom.setAttribute("id", "app-icon");
    icon.dom.addEventListener("pointerdown", (event) => {
      window.open("https://www.voxelo.ai/", "_blank").focus();
    });
